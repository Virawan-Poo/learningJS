...
const Home = () => {
  ...
  const groupUserByAge = () => {
    const groupAge = userData.reduce((prev, cur) => ({ ...prev, [cur.age]: (prev[cur.age] || []).concat(cur.name) }), {})
  ...
  const getUserByMaxAndMinAge = (action) => {
    ...
    if (action === 'max') {
      result = userData.reduce((prev, current) => (prev.age > current.age ? prev : current))
  ...
  return (
    <div>
      <button onClick={() => handleTotal(ENUM_ACTION.DECRESE)} disabled={count <= 0}>
        -
      </button>
      <span>{count}</span> <span>{text}</span>
      <button onClick={() => handleTotal(ENUM_ACTION.INCREASE)} disabled={count === 10}>
        +
      </button>
    </div>
  )
